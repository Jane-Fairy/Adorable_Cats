...
func (m *defaultGoodsSpecModel) Insert(ctx context.Context, data *GoodsSpec) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.SpecId, data.SpecName, data.DeleteTime, data.DelState)
	return ret, err
}
...
func (m *defaultGoodsSpecModel) Update(ctx context.Context, data *GoodsSpec) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.SpecName, data.DeleteTime, data.DelState, data.SpecId)
	return err
}
